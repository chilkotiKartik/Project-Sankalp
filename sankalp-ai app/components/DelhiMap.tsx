...
export default function DelhiMap({ complaints = [], sosAlerts = [], workers = [], policeStations = [], riskZones = [], filter = "all", userLocation, style }: Props) {
    ...
    <View style={[styles.container, style]}>
      ...
      >
        ...
        {show("risks") && riskZones.map(rz => (
          <Circle
            key={rz.id}
            center={{ latitude: rz.geo.lat, longitude: rz.geo.lng }}
            radius={rz.radius * 80}
            fillColor={RISK_COLORS[rz.type] + "22"}
            strokeColor={RISK_COLORS[rz.type] + "88"}
            strokeWidth={1.5}
          />
        ))}
        ...
        {show("risks") && riskZones.map(rz => (
          <Marker
            key={`risk-${rz.id}`}
            coordinate={{ latitude: rz.geo.lat, longitude: rz.geo.lng }}
            anchor={{ x: 0.5, y: 0.5 }}
          >
        ...
        {show("police") && policeStations.map(ps => (
          <Marker
            key={ps.id}
            coordinate={{ latitude: ps.geo.lat, longitude: ps.geo.lng }}
            anchor={{ x: 0.5, y: 1 }}
          >
        ...
        {show("workers") && workers.filter(w => w.geo && w.status === "active").map(w => (
          <Marker
            key={w.id}
            coordinate={{ latitude: w.geo!.lat, longitude: w.geo!.lng }}
            anchor={{ x: 0.5, y: 1 }}
          >
        ...
        {show("complaints") && complaints.slice(0, 120).map(c => (
          <Marker
            key={c.id}
            coordinate={{ latitude: c.geo.lat, longitude: c.geo.lng }}
            anchor={{ x: 0.5, y: 1 }}
          >
      ...
      <Pressable
        ...
        onPress={() => mapRef.current?.animateToRegion(DELHI_REGION, 500)}
      >
